...
Cross-game material lives at the repo root: the shared cars-as-trains mechanic write-up at [`/vehicle-stations.md`](./vehicle-stations.md), the vehicle-stations algorithm + CLI in [`/stations-generator/`](./stations-generator/), and the public site (rendered rules + in-browser generator at [ruralhs.ramblin.dev](https://ruralhs.ramblin.dev)) in [`/site/`](./site/).
...
### 🛠️ Tools & Scripts
- The JS code is an npm workspace rooted at the repo root: [`/stations-generator/core/`](./stations-generator/core/) (shared algorithm), [`/stations-generator/cli/`](./stations-generator/cli/) (Node CLI), and [`/site/`](./site/) (the Vite multi-page site, which embeds the generator at `/stations`). Run `npm install` from the repo root to get set up; `npm run dev` for the dev server, `npm test` to verify the regression test still passes, `npm run build` to produce `site/dist/` for Netlify.
